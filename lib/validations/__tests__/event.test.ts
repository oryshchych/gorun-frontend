...
describe('Event Validation Schemas', () => {
  ...
  describe('eventSchema', () => {
    ...
    it('should reject description longer than 2000 characters', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Description must not exceed 2000 characters');
      }
    });
    ...
    it('should reject past date', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Event date must be in the future');
      }
    });
    ...
    it('should reject location shorter than 3 characters', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Location must be at least 3 characters');
      }
    });
    ...
    it('should reject location longer than 200 characters', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Location must not exceed 200 characters');
      }
    });
    ...
    it('should reject non-integer capacity', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Capacity must be a whole number');
      }
    });
    ...
    it('should reject zero or negative capacity', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Capacity must be greater than 0');
      }
    });
    ...
    it('should reject capacity exceeding 10000', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toBe('Capacity must not exceed 10,000');
      }
    });
